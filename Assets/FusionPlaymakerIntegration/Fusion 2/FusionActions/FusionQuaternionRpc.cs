...
namespace GooglyEyesGames.PlayMaker.Fusion
{
	...
	public class FusionQuaternionRpc : FsmStateAction
	{
		[RequiredField]
		...
		public override void OnEnter()
        {
            ...
            {
                PlaymakerFusionNetworkRunner.PFNR.rpcHandler.TargetAll(remoteEvent.Name, quatData.Value);
            }
			else if(doNothingIfNull.Value == false)
            {
				PlaymakerFusionNetworkRunner.PFNR.rpcHandler.TargetObject((NetworkObject)networkObjectTarget.Value, remoteEvent.Name, quatData.Value);
			}
