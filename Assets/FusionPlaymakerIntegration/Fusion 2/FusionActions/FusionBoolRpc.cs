...
namespace GooglyEyesGames.PlayMaker.Fusion
{
	...
	public class FusionBoolRpc : FsmStateAction
	{
		[RequiredField]
		...
		public override void OnEnter()
        {
            ...
            {
                PlaymakerFusionNetworkRunner.PFNR.rpcHandler.TargetAll(remoteEvent.Name, boolData.Value);
            }
			else if (doNothingIfNull.Value == false)
            {
				PlaymakerFusionNetworkRunner.PFNR.rpcHandler.TargetObject((NetworkObject)networkObjectTarget.Value, remoteEvent.Name, boolData.Value);
			}
