...
namespace GooglyEyesGames.PlayMaker.Fusion
{
	...
	public class FusionIntRpc : FsmStateAction
	{
		[RequiredField]
		...
		public override void OnEnter()
        {
            ...
            {
                PlaymakerFusionNetworkRunner.PFNR.rpcHandler.TargetAll(remoteEvent.Name, intData.Value);
            }
			else if(doNothingIfNull.Value == false)
            {
				PlaymakerFusionNetworkRunner.PFNR.rpcHandler.TargetObject((NetworkObject)networkObjectTarget.Value, remoteEvent.Name, intData.Value);
			}
